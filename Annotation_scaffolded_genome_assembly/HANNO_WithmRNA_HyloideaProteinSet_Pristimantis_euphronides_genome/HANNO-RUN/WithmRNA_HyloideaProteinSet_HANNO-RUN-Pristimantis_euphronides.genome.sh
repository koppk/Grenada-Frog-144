...
## Helper: per-scaffold StringTie merge to avoid OOM
## Splits input into 4 balanced groups, merges in parallel, ensures unique MSTRG IDs
## Usage: stringtie_merge_safe <input.gtf> <output.gtf> [guide.gtf]
stringtie_merge_safe() {
    local input="$1"
    local output="$2"
    local guide="${3:-}"
    ...
    # Split input into 4 groups by one awk pass
    awk -F'\t' '
      $1=="scaffold_1" {print > "_grp1.gtf"; next}
      $1=="scaffold_2" || $1=="scaffold_3" || $1=="scaffold_5" {print > "_grp2.gtf"; next}
      /^scaffold_/ {print > "_grp3.gtf"; next}
      {print > "_grp4.gtf"}
    ' "$input"

    # If guide provided, split it too
    if [ -n "$guide" ]; then
        awk -F'\t' '
          $1=="scaffold_1" {print > "_guide_grp1.gtf"; next}
          $1=="scaffold_2" || $1=="scaffold_3" || $1=="scaffold_5" {print > "_guide_grp2.gtf"; next}
          /^scaffold_/ {print > "_guide_grp3.gtf"; next}
          {print > "_guide_grp4.gtf"}
        ' "$guide"
    fi

    # Run all 4 in parallel
    for i in 1 2 3 4; do
        if [ -s "_grp${i}.gtf" ]; then
            local garg=""
            if [ -n "$guide" ] && [ -s "_guide_grp${i}.gtf" ]; then
                garg="-G _guide_grp${i}.gtf"
            fi
            stringtie --merge $garg -o "_merged${i}.gtf" "_grp${i}.gtf" &
        fi
    done
    wait

    # Combine with unique MSTRG IDs per group (prefix group number to avoid collisions)
    > "$output"
    for i in 1 2 3 4; do
        if [ -f "_merged${i}.gtf" ]; then
            grep -v "^#" "_merged${i}.gtf" | sed "s/MSTRG\./MSTRG${i}_/g" >> "$output"
        fi
    done
    rm -f _grp?.gtf _merged?.gtf _guide_grp?.gtf
}

##RUN HANNO assembly pipeline:

mkdir WithmRNA_HyloideaProteinSet_HANNO-RUN-Pristimantis_euphronides.genome.fasta
cd WithmRNA_HyloideaProteinSet_HANNO-RUN-Pristimantis_euphronides.genome.fasta
##MAP PROTEINS in ../../GrenadaFrog144/Hyloidea_Proteins/combined_Hyloidea_proteins.faa
date
seqtk seq ../../../GrenadaFrog144/Pristimantis_euphronides.genome.fasta > asm.fa
...
##MERGE USING STRINGTIE -- per-scaffold to avoid OOM
echo "=== Per-scaffold StringTie merge: prot.merge.gtf ==="
stringtie_merge_safe asm.prot.gtf prot.merge.gtf
##MERGE USING TACO
...
##Stringtie -- per-scaffold merge to avoid OOM
echo "=== Per-scaffold StringTie merge: evidence.merge.gtf ==="
stringtie_merge_safe evidence.stringtie-like.gtf evidence.merge.gtf
echo "=== Per-scaffold StringTie merge: MODELS2.gtf ==="
cat evidence.merge.gtf output2/assembly.gtf > _combined_for_models2.gtf
stringtie_merge_safe _combined_for_models2.gtf MODELS2.gtf output2/assembly.gtf
rm -f _combined_for_models2.gtf

## NOTE: sed uses escaped \. to match only literal dots, preserving our MSTRGi_ prefixes
...
date
echo "=== HANNO pipeline with mRNA complete ==="
